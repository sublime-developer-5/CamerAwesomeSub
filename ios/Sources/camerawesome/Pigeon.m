  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:nullGetPigeonCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setManualIsoIso:error:)], @"CameraInterface api (%@) doesn't respond to @selector(setManualIsoIso:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        double arg_iso = [GetNullableObjectAtIndex(args, 0) doubleValue];
        FlutterError *error;
        [api setManualIsoIso:arg_iso error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
